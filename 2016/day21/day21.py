...
def do_swap_letters(s, x, y):
  t = str.maketrans(x + y, y + x)
  return s.translate(t)

def do_rotate_right(s, x):
  if len(s) < 1:
...
def do_rotate_position(s, x):
  idx = s.find(x)
  if x is None:
    raise ValueError("Can't find {} in {}".format(x, s))
  else:
    val = 1 + idx + (1 if idx >= 4 else 0)
    return do_rotate_right(s, val)

def do_reverse(s, x, y):
  if x > y:
    x, y = y, x
  if x >= len(s) or y >= len(s):
