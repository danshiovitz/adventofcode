...
use common::grid::{Coord, Direction, add_direction};
...
fn fire_probe(start: Coord, dir: Direction, area: Area, verbose: bool) -> Option<i32> {
    ...
    let mut max_y = 0;
    if verbose {
        println!("Firing probe at ({},{}) trying to hit ({},{}..{},{})", dir.dx, dir.dy, area.min.x, area.min.y, area.max.x, area.max.y);
    }
    while cur.y + cur_dir.dy >= area.min.y {
        ...
        cur_dir.dy -= 1;
        if verbose {
            println!("Probe is at ({},{}), dir is ({},{})", cur.x, cur.y, cur_dir.dx, cur_dir.dy);
        }
...
impl BaseDay for Day17 {
    fn parse(&mut self, input: &mut InputReader) {
        fn parse_line(line: String) -> Area {
            ...
            match rex.captures(&line) {
                Some(c) => {
                    return Area {
                        min: Coord { x: c[1].parse::<i32>().unwrap(), y: c[3].parse::<i32>().unwrap() },
                        max: Coord { x: c[2].parse::<i32>().unwrap(), y: c[4].parse::<i32>().unwrap() },
                    };
                }
    ...
    fn pt1(&mut self) -> String {
        ...
        let ret = dxs.cartesian_product(dys).filter_map(|(dx, dy)| fire_probe(start, Direction { dx: dx, dy: dy }, area, false)).max().unwrap();
    ...
    fn pt2(&mut self) -> String {
        ...
        let dys: Range<i32> = area.min.y - 1..area.max.x + 1;
        let start = Coord { x: 0, y: 0 };
        let ret = dxs.cartesian_product(dys).filter_map(|(dx, dy)| fire_probe(start, Direction { dx: dx, dy: dy }, area, false)).count();
        return ret.to_string();
    }
}
